...
with cte as(
SELECT u.user_id,u.event_type,u.event_date,
EXTRACT(month from u.event_date) as month,
u2.event_type,u2.event_date
FROM user_actions u, user_actions u2
where u.user_id=u2.user_id and
EXTRACT(year from u.event_date) = EXTRACT(year from u2.event_date) and
EXTRACT(month from u.event_date) = 7 
and EXTRACT(month from u2.event_date) = 6)
select month, count(DISTINCT user_id) monthly_active_users 
